...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(option =>
    {
        ...
        option.LoginPath = "/ShopAccount/Login";        // 👈 Chuyển đến trang đăng nhập
...
builder.Services.AddAuthorization(options =>
{
    options.FallbackPolicy = new AuthorizationPolicyBuilder()
        .RequireAuthenticatedUser()     // 👈 Yêu cầu tất cả người dùng phải đăng nhập
        .Build();
});
...
SV22T1020283.BusinessLayers.Configuration.Initialize(connectionString);
